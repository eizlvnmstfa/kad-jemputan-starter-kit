...
const tabButtonClass = (name) => {
    const activeClass = props.theme === 'bride'
        ? 'bg-white/85 text-brideTertiary shadow-inner'
        : 'bg-white/80 text-groomTertiary shadow-inner'
    const inactiveClass = props.theme === 'bride'
        ? 'text-brideSecondary/70 hover:bg-white/55 hover:text-brideTertiary'
        : 'text-groomSecondary/70 hover:bg-white/55 hover:text-groomTertiary'

    return [
        'flex min-h-20 flex-col items-center justify-center gap-2 px-2 py-4 transition-colors duration-300',
        tab.value === name ? activeClass : inactiveClass,
    ]
}

const tabIconClass = (name) => {
    const activeClass = props.theme === 'bride' ? 'text-brideTertiary' : 'text-groomTertiary'
    const inactiveClass = props.theme === 'bride'
        ? 'text-brideSecondary/60 group-hover:text-brideTertiary'
        : 'text-groomSecondary/60 group-hover:text-groomTertiary'
...
<template>
    <div ref="footerPanelRef" class="sticky bottom-0 z-50 overflow-hidden rounded-t-3xl border-t text-center shadow-[0_-14px_40px_rgba(0,0,0,0.10)] backdrop-blur-md" :class="props.theme === 'bride' ? 'border-brideTertiary/15 bg-[#FBF7F2]' : 'border-groomTertiary/15 bg-[#FBF7F2]'">
        <div :class="['transition-all duration-500', tab == '' ? 'hidden' : 'p-6 pb-5']">
            <button type="button" aria-label="Tutup panel footer" class="absolute right-4 top-4 z-10 flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border bg-white shadow-lg transition-all hover:scale-105 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2" :class="props.theme === 'bride' ? 'border-brideTertiary/30 text-brideTertiary hover:bg-brideSecondary focus-visible:ring-brideTertiary/70' : 'border-groomTertiary/30 text-groomTertiary hover:bg-groomSecondary focus-visible:ring-groomTertiary/70'" @click="changeTab('')">
            ...
            <card-footer-calendar v-if="tab == 'calendar'" :calendar="props.footerDetails.calendar" :theme="props.theme"/>
            <card-footer-contact v-if="tab == 'contact'" :contacts="props.footerDetails.contacts" :theme="props.theme"/>
            <card-footer-map v-if="tab == 'map' && props.footerDetails.maps.iframe" :maps="props.footerDetails.maps" :theme="props.theme"/>
        </div>
        <div class="grid grid-cols-3 divide-x border-t bg-white/40 text-white fill-current" :class="props.theme === 'bride' ? 'divide-brideTertiary/10 border-brideTertiary/10' : 'divide-groomTertiary/10 border-groomTertiary/10'">
            <div v-for="button in tabButtons" :key="button.name" class="group cursor-pointer" :class="tabButtonClass(button.name)" @click="changeTab(button.name)">
                ...
                <span class="text-[10px] uppercase tracking-[0.28em] transition-colors duration-300" :class="tab === button.name ? (props.theme === 'bride' ? 'text-brideTertiary' : 'text-groomTertiary') : (props.theme === 'bride' ? 'text-brideSecondary/65 group-hover:text-brideTertiary' : 'text-groomSecondary/65 group-hover:text-groomTertiary')">{{ button.label }}</span>
            </div>
        </div>
    </div>
</template>
