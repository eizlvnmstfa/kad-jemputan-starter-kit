...
<template>
    <div class="sticky bottom-0 z-50 overflow-hidden rounded-t-3xl border-t border-groomTertiary/15 bg-[#FBF7F2] text-center shadow-[0_-14px_40px_rgba(0,0,0,0.10)] backdrop-blur-md">
        <div :class="['transition-all duration-500', tab == '' ? 'hidden' : 'p-6 pb-5']">
            <div class="absolute right-0 top-0 mr-6 -mt-3 flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border border-groomTertiary/20 bg-white text-groomTertiary shadow-lg transition-transform hover:scale-110" @click="changeTab('')">
            ...
            <card-footer-rsvp v-if="tab == 'rsvp' && props.footerDetails.rsvp.iframe" :rsvp="props.footerDetails.rsvp"/>
        </div>
        <div class="grid grid-cols-4 divide-x divide-groomTertiary/10 border-t border-groomTertiary/10 bg-white/40 text-white fill-current">
